...
// Porao App — Invite Friends স্ক্রিন
// Drop this into: src/screens/InviteFriendsScreen.js
// AppNavigator.js-এ একটা স্ট্যাক স্ক্রিন হিসেবে যোগ করো, নাম দাও "InviteFriends"
...
export default function InviteFriendsScreen() {
  const { user } = useContext(AuthContext); // তোমার AuthContext থেকে current user
  ...
  useEffect(() => {
    ...
    (async () => {
      if (!user) return;
      const code = await ensureReferralCode(user.uid, user.displayName);
      const data = await getReferralStats(user.uid);
      if (mounted) {
        setStats(data || { code, count: 0, points: 0 });
        setLoading(false);
      }
    })();
    return () => {
  ...
  return (
    <View style={styles.container}>
      ...
      <Text style={styles.subtext}>
        প্রতি বন্ধু জয়েন করলে তুমি পাবে {10} পয়েন্ট, আর ৫ জন হলেই বোনাস!
      </Text>
